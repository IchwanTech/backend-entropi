...
export class SettlementService {
  ...
  async settle(date: string): Promise<SettlementResult> {
    ...
    const existingSettlement = await this.db.eventLog.findUnique({ where: { idempotencyKey } })
    ...
    if (orders.length === 0) {
      return { date, totalOrders: 0, totalRevenue: '0.0000', totalFees: '0.0000', totalPayout: '0.0000', idempotent: false }
    }
