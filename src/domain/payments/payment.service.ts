...
export class PaymentService {
  ...
  async processPayment(params: {
    ...
    await this.db.$transaction(async (tx: any) => {
      ...
      // Kalau count = 0 berarti version sudah berubah (concurrent request menang)
      // Skip saja — idempotency key check di atas akan handle retry
      // If an event with the same aggregateId and version already exists, skip creation.
      // This handles cases where the event might have been processed but the
      // transaction failed before committing, leading to a retry.
      if (result.count === 0) {
        await tx.eventLog.create({
          data: {
            ...
            payload: {
              ...
              stripeIdempotencyKey: `${idempotencyKey}-stripe`,
            },
